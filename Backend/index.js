...
const Signin = require('./DB/Sigin');
const Sigin = require('./DB/Sigin');
const app = express();
...
app.post('/signup', async(req, res) => {
    try {
        ...
        res.status(201).json({ message: 'Sign-in successful', user: newUser });
    } catch (error) {
...
app.get('/signin/data', async(req, res) => {
    try {
        const purchase = await Sigin.find(); // Use Customer model to find customers
        res.json(purchase);
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'Server error' });
